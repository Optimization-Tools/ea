...
class fitness_proportionate(selection_mechanism):
	...
	def select(parents):
		parents.set_individuals(roulette([(parent, parent.fitness()) for parent in parents.get_individuals], this.n))
	
class stochastic_uniform(selection_mechanism):
	def __init__(this, n):
		super(stochastic_uniform, this).__init__(n)
	def select(parents):
		parents.set_individuals(roulette([(parent, 1) for parent in parents.get_individuals], this.n))
...
class sigma_scaling(selection_mechanism):
	...
	def select(parents):
		...
		parents.set_individuals(roulette([(parent, (value-average)/(2*sigma)) for parent, value in parents], this.n))
